...
class ExcelComparisonApp:
    ...
    def _show_help_window(self, section):
        ...
        elif section == "mode_selection":
            text_area.insert(tk.END, "Comparison Mode Selection\n\n", "heading")
            text_area.insert(tk.END, "This tool offers two different modes for comparing and updating Excel files:\n\n", "normal")
            text_area.insert(tk.END, "• Standard Mode: ", "subheading")
            text_area.insert(tk.END, "Uses a fixed structure with Team, App Name, and Category columns for matching rows between files. "
                             "This is ideal when your Excel files follow this specific structure.\n\n", "normal")
            text_area.insert(tk.END, "• Custom Mode: ", "subheading") 
            text_area.insert(tk.END, "Allows you to select any columns as 'key columns' for matching rows between files. "
                             "Use this mode when your Excel files use different column structures or when you need more flexibility "
                             "in how rows are matched between files.\n\n", "normal")
            text_area.insert(tk.END, "Both modes support filtering and formula preservation.", "normal")
        
        elif section == "custom_comparison_criteria":
            text_area.insert(tk.END, "Custom Comparison Criteria\n\n", "heading")
            text_area.insert(tk.END, "This section lets you define how rows are matched between files in custom mode:\n\n", "normal")
            text_area.insert(tk.END, "• Header Row: ", "subheading")
            text_area.insert(tk.END, "The row number containing column headers (default is 4).\n\n", "normal")
            text_area.insert(tk.END, "• Formula-Aware Processing: ", "subheading")
            text_area.insert(tk.END, "When enabled, the tool will preserve formulas in the destination file.\n\n", "normal") 
            text_area.insert(tk.END, "• Key Columns: ", "subheading")
            text_area.insert(tk.END, "These are the columns used to match rows between files. A row in the old file will be "
                             "updated with data from a row in the new file when all key column values match.\n\n", "normal")
            text_area.insert(tk.END, "• Add Key Column: ", "subheading")
            text_area.insert(tk.END, "Click to add more columns to use for matching. More columns create more specific matches.\n\n", "normal")
            text_area.insert(tk.END, "• Remove: ", "subheading")
            text_area.insert(tk.END, "Click to remove a key column (you must have at least one).\n\n", "normal")
            text_area.insert(tk.END, "Click 'Load Columns' to populate the dropdown lists after selecting sheets.", "normal")
        
        elif section == "custom_filter_criteria":
            text_area.insert(tk.END, "Custom Filter Criteria\n\n", "heading")
            text_area.insert(tk.END, "This section allows you to filter which rows are updated in custom mode:\n\n", "normal")
            text_area.insert(tk.END, "• Add Custom Filter: ", "subheading")
            text_area.insert(tk.END, "Click this button to create a new filter based on any column. When you add a filter, "
                             "only rows that match at least one of the filter values for each filtered column will be updated.\n\n", "normal")
            text_area.insert(tk.END, "• Filter Values: ", "subheading") 
            text_area.insert(tk.END, "Each filter can have multiple values (which work as OR logic - a row matches if it equals any of the values).\n\n", "normal")
            text_area.insert(tk.END, "• Get Values (🔍): ", "subheading")
            text_area.insert(tk.END, "Click the magnifying glass icon to see and select from all possible values in the selected sheet.\n\n", "normal")
            text_area.insert(tk.END, "• Add Value (+): ", "subheading")
            text_area.insert(tk.END, "Click the plus button to add another filter value for the same column.\n\n", "normal")
            text_area.insert(tk.END, "• Remove Value (❌): ", "subheading")
            text_area.insert(tk.END, "Click the X button to remove a filter value.\n\n", "normal")
            text_area.insert(tk.END, "Leave all filters empty to include all rows that match the key columns.", "normal")
            ...
            for sheet_name in selected_sheets:
                ...
                for row in range(header_row + 1, new_sheet.max_row + 1):
                    ...
                    for key_col in key_columns:
                        if key_col in headers:  # Ensure the column exists in the new file
                            ...
                            # If column doesn't exist in new file, use empty value
                            key_parts.append("")
                    
                    # Create composite key - must have same number of parts as old keys
                    if len(key_parts) == len(key_columns):
                        key = "|".join(key_parts)
                ...
                for key in filtered_keys:
                    ...
                    for col in range(1, min(old_sheet_raw.max_column, new_sheet.max_column) + 1):
                        ...
                        if col_to_name.get(col) in key_columns:
                            continue
                        ...
                        if col in col_to_name:
                            ...
                            if old_cell.value != new_cell.value:
                                ...
                                # Update while preserving comments
                                self._update_cell_preserve_comments(new_cell, old_cell)
                                cells_updated = True
