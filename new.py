...
class ExcelComparisonApp:
    ...
    def _create_filter_widgets(self, parent):
        ...
        h_canvas.bind('<Configure>', lambda e: h_canvas.itemconfig(canvas_window, width=e.width))
        
        # Configure scrolling based on content width
        default_filters_container.bind(
            "<Configure>",
            lambda e: h_canvas.configure(scrollregion=(0, 0, e.width, e.height))
        )
        
        # Create individual filter frames for each default type within the scrollable container
        ...
        # Create vertical scrollable frame for all filters (including additional ones)
        ...
        # Add mouse wheel binding for horizontal scrolling
        def _on_mousewheel_horizontal(event):
            h_canvas.xview_scroll(int(-1*(event.delta/120)), "units")
        
        # Add mouse wheel binding for vertical scrolling
        def _on_mousewheel_vertical(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        
        # Bind mouse wheel events
        h_canvas.bind("<Enter>", lambda e: h_canvas.bind_all("<Shift-MouseWheel>", _on_mousewheel_horizontal))
        h_canvas.bind("<Leave>", lambda e: h_canvas.unbind_all("<Shift-MouseWheel>"))
        
        canvas.bind("<Enter>", lambda e: canvas.bind_all("<MouseWheel>", _on_mousewheel_vertical))
    ...
    def _refresh_filter_widgets(self):
        ...
        for i, filter_var in enumerate(self.team_filters):
            entry = ttk.Entry(self.team_filter_frame, textvariable=filter_var, width=col_width)
        ...
        for i, filter_var in enumerate(self.app_name_filters):
            entry = ttk.Entry(self.app_filter_frame, textvariable=filter_var, width=col_width)
            entry.pack(fill="x", padx=5, pady=2)
            ttk.Button(
        ...
        for i, filter_var in enumerate(self.category_filters):
            entry = ttk.Entry(self.category_filter_frame, textvariable=filter_var, width=col_width)
            entry.pack(fill="x", padx=5, pady=2)
            ttk.Button(
        ...
        for criteria_label, filter_frame in self.additional_filter_frames.items():
            ...
            for i, filter_var in enumerate(filters):
                entry = ttk.Entry(filter_frame, textvariable=filter_var, width=col_width)
                entry.pack(fill="x", padx=5, pady=2)
                ttk.Button(
                    filter_frame, 
                    text="Get Values", 
